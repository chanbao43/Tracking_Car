...
// ============ 循迹计数 ============
uchar Line = 0;
uchar Line_SM = 0;  // 初始 = SM_WAIT_BLACK SM状态机
uchar Debounce_Cnt = 0;//去除抖动器计数器
        ...
        case SM_WAIT_WHITE://在白线上状态
            if(left == 1 && right == 1)
            {
                Debounce_Cnt++;//去除抖动计数器
							if(Debounce_Cnt >= DEBOUNCE_MS)//如果去除抖动计数次数>=去抖时间，即一直在黑线上
                {
                    Line++;//线数+1
