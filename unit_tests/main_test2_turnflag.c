/* ============================================================
 * Test 2: 两段式计数 (Turn_Flag)
 * 基于: Test 1 (两位显示)
 * 新增: Turn_Flag 变量, 掉头后不重置 Line, 继续从 8 往后数
 * 验证: 计数 1→8→[掉头]→9→10→... 数码管显示正确
 * ============================================================ */
#include <reg51.h>
...
// [NEW] 两段式标志
bit  Turn_Flag = 0;                // 0=第一段(去程), 1=第二段(返程)
